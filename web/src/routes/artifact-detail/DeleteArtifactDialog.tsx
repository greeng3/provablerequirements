        ...
        >
            ...
            >
                ...
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
                    This removes the artifact file from disk. The action can't
                    be undone from within Provreq — use git to recover.
                </p>
                ...
                {incoming.isLoading ? (
                    ...
                    <div className="mt-3 rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100">
                        <p className="font-medium">
                            ...
                            {count === 1 ? "" : "s"} link
                            {count === 1 ? "s" : ""} to this one. Proceeding
                            will leave those links unresolved (Provreq does not
                            auto-rewrite sources).
                        </p>
                        <ul className="mt-2 space-y-0.5 font-mono text-xs">
                            {(incoming.data ?? []).slice(0, 10).map((link) => (
                                <li key={`${link.sourceUuid}:${link.linkType}`}>
                                    {link.projectSlug}/{link.collectionPrefix}/
                                    {link.artifactName}{" "}
