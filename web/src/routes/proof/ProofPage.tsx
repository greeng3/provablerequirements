...
/// Grafted into the Provreq management frontend as `/proof` and restyled to the
        ...
        <section className="space-y-6">
            <header className="space-y-1 border-b border-slate-200 pb-3 dark:border-slate-800">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    The requirement backlog and its formalization coverage.
                    Triage each item, then verify the formalized ones against
                    the engine ensemble.
                </p>
            </header>
...
function BacklogView({ backlog }: { backlog: ProofBacklog }) {
    ...
    const handleReverifyStale = async () => {
        ...
        if (failures.length > 0) {
            setReverifyError(
                `Some re-verifications failed — ${failures.join("; ")}`,
            );
        }
    };
                ...
                {backlog.coverage.untriaged > 0 && (
                    <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
                        <button
                            type="button"
                            onClick={() =>
                                seed.mutate({ reclassify: false, repeat: true })
                            }
                        ...
                        <span className="text-xs text-slate-500">
                            ...
                            set — the same as <code>provreq triage --repeat</code>.
                            Keeps re-asking the still-untriaged items until the
                            backlog converges, so one click finishes it.
                        </span>
                    </div>
                )}
