...
/// but leaves provreq.json + the artifacts/ root in place. The
/// "also de-initialize" checkbox widens the blast radius to remove
/// provreq.json and the artifacts/ directory itself, reverting
        ...
        >
            ...
            >
                ...
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <span className="font-mono">{projectSlug}</span> along with
                    all artifacts inside them. The Project itself (
                    <span className="font-mono">provreq.json</span>) is left
                    untouched so the Project continues to load — but every
                    collection, artifact, and link will be gone.
                </p>
                {deinit ? (
                    <p
                    ...
                    >
                        ...
                        <span className="font-mono">provreq.json</span> and the{" "}
                        <span className="font-mono">artifacts/</span> directory
                        itself are also removed, so this repo will revert to a
                        NeedsInit mount as if Provreq had never touched it.
                    </p>
                ) : null}
                <p className="mt-2 text-sm font-semibold text-rose-700 dark:text-rose-400">
                    This cannot be undone from the UI. Make sure you have a
                    commit (or can{" "}
                    <span className="font-mono">git restore</span>) before
                    proceeding.
                </p>
