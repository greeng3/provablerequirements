        ...
        >
            <p className="font-semibold">
                {diagnostics.length === 1
                    ? "One file was written by a newer Provreq"
                    : `${diagnostics.length} files were written by a newer Provreq`}
            </p>
            <p className="mt-1">
                These files won't load until you upgrade Provreq. Other files in
                the project continue to work.
            </p>
            <ul className="mt-2 space-y-1 font-mono text-xs">
                {diagnostics.map((d, idx) => (
                    <li key={`${d.path}-${idx}`}>
                        {d.path} · <span className="italic">{d.fileType}</span>{" "}
                        · v{d.foundVersion} (this build supports up to v
                        {d.currentVersion})
                    </li>
                ))}
            </ul>
