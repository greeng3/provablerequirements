...
function mount(onClose = () => {}) {
    return render(
        <TestQueryProvider>
            <MemoryRouter>
                <WipeArtifactsDialog projectSlug="provreq" onClose={onClose} />
            </MemoryRouter>
        </TestQueryProvider>,
    );
}
...
describe("WipeArtifactsDialog", () => {
    ...
    it("disables the wipe button until the slug is typed exactly", async () => {
        ...
        await userEvent.type(input, "provreq");
    ...
    it("DELETEs without ?deinit when the checkbox is unchecked", async () => {
        const calls = stubFetch((url, method) => {
            if (
                method === "DELETE" &&
                url.includes("/api/projects/provreq/artifacts")
            ) {
        ...
        expect(deleteCall!.url).toContain("/api/projects/provreq/artifacts");
    ...
    it("DELETEs with ?deinit=true and adapts the heading + button label when the checkbox is checked", async () => {
        ...
        await userEvent.click(
            screen.getByTestId("wipe-artifacts-deinit-checkbox"),
        );
        expect(
        ...
        expect(deleteCall!.url).toContain(
            "/api/projects/provreq/artifacts?deinit=true",
        );
    });
    ...
    it("surfaces a server error and keeps the dialog open", async () => {
        stubFetch((url, method) => {
            if (
                method === "DELETE" &&
                url.endsWith("/api/projects/provreq/artifacts")
            ) {
                return {
                    status: 500,
                    body: { error: "wipe collection dirs: nope" },
                };
            }
        ...
        await waitFor(() =>
            expect(
                screen.getByTestId("wipe-artifacts-error"),
            ).toBeInTheDocument(),
        );
