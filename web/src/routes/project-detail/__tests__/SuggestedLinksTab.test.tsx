...
function mount() {
    return render(
        <TestQueryProvider>
            <MemoryRouter>
                <SuggestedLinksTab projectSlug="provreq" />
            </MemoryRouter>
        </TestQueryProvider>,
    );
}
...
describe("SuggestedLinksTab", () => {
    ...
    it("lists pending suggestions and POSTs accept on click", async () => {
        ...
        await waitFor(() => {
            ...
            expect(acceptCall!.url).toContain(
                "/api/projects/provreq/suggestions/links/0194f6d0-0001-7000-8000-000000000001/accept",
            );
        });
    });
    ...
    it("switches to rejected sub-tab and reinstates a declined suggestion", async () => {
        ...
        await userEvent.click(
            screen.getByTestId("suggested-links-rejected-tab"),
        );
        await waitFor(() =>
            expect(
    ...
    it("shows the noProviders banner when analyze returns kind=noProviders", async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByTestId("analyze-link-suggestions"),
            ).toBeEnabled(),
        );
