        ...
        >
            ...
            >
                ...
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                    One-way import: doorstop source files are parsed, planned,
                    and written as new Provreq collections. The originals are
                    never modified.
                </p>
        ...
        <div className="mt-4 space-y-4">
            ...
            >
                <p>
                    ...
                    {totalArtifacts === 1 ? "" : "s"} ·{" "}
                    <strong>{plan.unresolvedLinks.length}</strong> unresolved
                    link
            ...
            {hasCollisions ? (
                ...
                >
                    <p className="font-semibold">
                        Prefix collision — import blocked.
                    </p>
                    <p className="mt-1">
                        Resolve by renaming one of the prefixes or removing the
                        conflicting Provreq collection, then re-preview.
                    </p>
                ...
                <details className="rounded border border-amber-300 bg-amber-50 p-3 text-xs dark:border-amber-700 dark:bg-amber-900/30">
                    ...
                    <ul className="mt-2 space-y-1 font-mono">
                        {plan.unresolvedLinks.map((u, idx) => (
                            <li key={idx}>
                                {u.sourceUid} → <code>{u.targetUid}</code> (not
                                imported)
                            </li>
                        ))}
                    </ul>
    ...
    return (
        <div className="mt-4 space-y-4" data-testid="doorstop-report-panel">
            <div className="rounded border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-900 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-100">
                ...
                {report.totals.artifactsImported === 1 ? "" : "s"} written
                across {report.totals.collectionsCreated} collection
            ...
            <dl className="grid grid-cols-2 gap-2 text-sm">
                ...
                <ReportStat
                    label="URL artifacts"
                    value={report.totals.urlArtifacts}
                />
                <ReportStat
                    label="cites links"
                    value={report.totals.citesLinks}
                />
                <ReportStat
                    label="Legacy refs"
                    value={report.totals.legacyRefs}
                />
                <ReportStat
