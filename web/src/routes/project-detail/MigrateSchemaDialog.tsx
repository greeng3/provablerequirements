            ...
            { force },
            {
                ...
                onError: (err) => {
                    if (err instanceof ApiError && err.status === 409) {
                        ...
                        setDirtyWarning(
                            body?.error ??
                                "Project worktree has uncommitted changes.",
                        );
                    }
                },
            },
        ...
        >
            ...
            >
                ...
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                    Walks every Provreq-authored file in{" "}
                    <span className="font-mono">{projectSlug}</span> and
                    rewrites any file whose <code>schemaVersion</code> is below
                    the current. Provreq never commits — stage + commit the
                    rewrites yourself so the migration lands as its own change.
                </p>
                        ...
                        {dirtyWarning ? (
                            ...
                            >
                                <p className="font-semibold">
                                    The project worktree has uncommitted
                                    changes.
                                </p>
                                <p className="mt-1">
                                    Commit them first so the migration lands as
                                    its own commit, or proceed anyway and mix
                                    them together.
                                </p>
        ...
        <div className="mt-4 space-y-3" data-testid="migrate-schema-result">
            {everythingCurrent ? (
                <div className="rounded border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-900 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-100">
                    Every one of the {result.filesScanned} Provreq-authored file
                    {result.filesScanned === 1 ? "" : "s"} is already at the
                    current schema. Nothing was rewritten.
                </div>
            ) : (
                ...
                >
                    ...
                    <ul className="mt-2 space-y-1 font-mono">
                        {result.rewritten.map((r, idx) => (
                            <li key={`${r.path}-${idx}`}>
                                {r.path} · v{r.outcome.fromVersion} → v
                                {r.outcome.toVersion}
                            </li>
                        ))}
                    </ul>
                ...
                >
                    ...
                    <ul className="mt-2 space-y-1 font-mono">
                        {result.failures.map((f, idx) => (
                            <li key={`${f.path}-${idx}`}>
                                {f.path} ·{" "}
                                <span className="italic">{f.fileType}</span>:{" "}
                                {f.error}
                            </li>
                        ))}
                    </ul>
