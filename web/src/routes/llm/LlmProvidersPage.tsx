    ...
    return (
        <section aria-labelledby="llm-heading" className="space-y-4">
            <header className="flex flex-wrap items-baseline justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                        Provreq walks this list top-to-bottom when a feature
                        needs an LLM, skipping disabled entries. On failure it
                        falls through; permanent failures (auth, unknown model)
                        hard-disable the slot for this process until a retest.
                    </p>
                </div>
                <button
                    type="button"
            ...
            {providers.isLoading ? (
                ...
                <p
                    data-testid="llm-providers-empty"
                    className="text-sm text-slate-500"
                >
                    No LLM providers configured. Click{" "}
                    <strong>Add provider</strong> above to register one —
                    entries persist to{" "}
    ...
    return (
        ...
        >
            ...
            <p className="mt-1 text-xs text-slate-500">
                Fields persist to <span className="font-mono">system.json</span>{" "}
                at mode 0600. Leave the API key blank for keyless providers
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className="text-sm">
                    ...
                    <select
                        value={provider}
                        onChange={(e) =>
                            setProvider(e.target.value as ProviderFamily)
                        }
