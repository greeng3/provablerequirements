...
// backend/provreq-server/src/http/dto.rs. When the backend wire
...
export type ReviewState =
    "neverReviewed" | "approved" | "rejected" | "reRequested";
...
export type ReportScopeParam =
    "system" | `project:${string}` | `collection:${string}/${string}`;
...
/// `backend/provreq-server/src/graph/mod.rs`; the frontend uses
...
/// `backend/provreq-server/src/search/query.rs`.
...
// the Provreq server and keeps its original serde wire shape: snake_case
// coverage counts and kebab-case enum values. Unlike the camelCase Provreq
...
export type ProofClassification =
    "formalizable-now" | "falsifiable-only" | "stays-prose";
...
export type ProofEngineState =
    "available" | "missing" | "unusable" | "incompatible" | "not-wired";
