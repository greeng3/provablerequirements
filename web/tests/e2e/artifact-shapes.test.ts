...
describe("Artifact shapes smoke suite", async () => {
    ...
    it("renders the blob artifact with Download + Replace file actions", async () => {
        await driver.get(
            `${env.provreqUrl}/projects/sample-project/collections/DES/artifacts/DES-logo`,
        );
        await driver.wait(
        ...
        const title = await driver
            ...
            .getText();
        assert.equal(title, "Provreq logo");
        ...
        const previewImg = await driver.wait(
            until.elementLocated(
                By.css("img[src*='/api/artifacts/'][src*='/blob']"),
            ),
            env.stepTimeoutMs,
        );
    ...
    it("renders the URL artifact with a clickable link and Check URL now", async () => {
        await driver.get(
            `${env.provreqUrl}/projects/sample-project/collections/REF/artifacts/REF-rfc9110`,
        );
        await driver.wait(
            until.elementLocated(By.css("h1#artifact-heading")),
            env.stepTimeoutMs,
    ...
    it("routes to the standalone diff view when navigating to /artifacts/:uuid/diff", async () => {
        ...
        await driver.get(`${env.provreqUrl}/artifacts/${uuid}/diff`);
        await driver.wait(
