...
describe("Provreq review-workflow smoke", async () => {
    const env = readEnv();
    ...
    it("drives the five review actions against the sample fixture", async () => {
        ...
        await driver.get(`${env.provreqUrl}/reviews`);
        await driver.wait(
        ...
        await driver.get(
            `${env.provreqUrl}/artifacts/0194f6d0-0001-7000-8000-000000000002`,
        );
        await driver.wait(
            until.elementLocated(
                By.css("section[aria-labelledby='review-pane-heading']"),
...
async function clickByText(driver: WebDriver, text: string): Promise<void> {
    const button = await driver.wait(
        until.elementLocated(
            By.xpath(
                `//button[not(@disabled) and normalize-space()='${text}']`,
            ),
        ),
        5_000,
    );
