...
make check-requirements    # validate the Provreq requirements project
...
Requirements are provreq-native, stored in [`requirements/`](requirements) (a
`provreq.json` manifest plus one artifact per requirement under `artifacts/req/`).
...
in a provreq-native store (importing an existing Doorstop tree once), triages them
