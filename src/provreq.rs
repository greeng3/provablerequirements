//! The Provreq [`RequirementsSource`] adapter (adapter #2) — began as the spike for #296.
//!
//! [`crate::source`] drew this seam single-implementation and said why: "the provreq adapter is a
...
//! a requirement from Provreq's on-disk shape and seeing what that dragged in. The thesis held:
//! reading a requirement moved no engine adapter, refusal classification, mirror channel, or verdict
//! model.
//!
//! It now reads and writes through the extracted `provreq-model` crate (#305): `items()` loads with
//! Provreq's own loader, and `annotate()` appends provenance to an artifact's review log with its
...
/// The file marking a directory as a Provreq collection. Its presence is what tells `adopt` the
...
/// Reads requirement prose from a subject's Provreq artifacts.
pub struct ProvreqSource {
    root: PathBuf,
}
...
impl RequirementsSource for ProvreqSource {
    fn items(&self) -> Result<Vec<Item>> {
        ...
        for dir in discover(&self.root) {
            ...
            for entry in entries {
                ...
                // Read through Provreq's own loader (#305). The spike hand-rolled a frontmatter
                ...
                // The filename stem is the identity Provreq itself uses (`LoadedArtifact::name`),
                ...
                items.push(Item {
                    // `modifiedAt` is Provreq's native token, and is *not* used: it moves when
                    ...
                    // seeds a bucket. Provreq defines this flag (`TRACE-codeCoverageExpectation`)
                    ...
                    // real information — Provreq's generalisation of doorstop's non-functional
    ...
    ///
    /// Provreq's review log is where an admission belongs — the phase-1 reason this refused until
    ...
    fn annotate(&self, id: &str, annotation: &Annotation) -> Result<()> {
        ...
        // Provreq outcome tag's own fields.
        ...
        let mut overflow = provreq_model::schema::Overflow::new();
        overflow.insert("provreq".into(), provreq);

        self.append_review_entry(
            id,
            provreq_model::schema::ReviewLogEntry {
                timestamp,
...
impl ProvreqSource {
    ...
    /// two provreq breadcrumbs read as one provenance family and neither can collide with a Provreq
    ...
    fn append_review_entry(
        &self,
        id: &str,
        entry: provreq_model::schema::ReviewLogEntry,
    ) -> Result<()> {
        let path = self.artifact_path(id)?;
        let loaded = provreq_model::load::artifact::load_content_artifact(&path)
            .with_context(|| format!("loading {}", path.display()))?;
        ...
        let bytes = provreq_model::write::render_artifact_file(&metadata, &body)
            .with_context(|| format!("rendering {}", path.display()))?;
        provreq_model::write::atomic_write(&path, &bytes)
    ...
    fn artifact_path(&self, id: &str) -> Result<PathBuf> {
        ...
        bail!(
            "no Provreq artifact with id {id} in {}",
            self.root.display()
        )
    }
}
...
mod tests {
    ...
    /// The committed fixture, copied verbatim from a real artifact in Provreq's own tree.
    fn fixture() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/provreq-subject")
    }

    // Verifies: REQ009 / #296 — a Provreq artifact reaches the seam as an ordinary `Item`. The
    // fixture is the real on-disk shape, JSON frontmatter and all, so this fails if that format is
    // read from documentation rather than from what Provreq actually writes.
    #[test]
    fn reads_a_provreq_artifact_as_a_source_item() {
        let items = ProvreqSource::new(fixture()).items().unwrap();
    ...
    /// the on-disk shape stays the one Provreq actually writes, not a hand-rolled guess.
    fn items_with_expects(expects: Option<bool>) -> Vec<Item> {
        ...
        .unwrap();
        ProvreqSource::new(tmp.path()).items().unwrap()
    }
    ...
    fn the_revision_follows_the_prose_and_not_the_timestamp() {
        ...
        let before = ProvreqSource::new(tmp.path()).items().unwrap()[0]
            .revision
            .clone();
        ...
        assert_eq!(
            ProvreqSource::new(tmp.path()).items().unwrap()[0].revision,
            before,
    ...
    fn a_mac_sidecar_never_becomes_a_requirement() {
        ...
        let items = ProvreqSource::new(tmp.path()).items().unwrap();
    ...
    fn an_inactive_artifact_is_not_a_requirement() {
        ...
        assert!(ProvreqSource::new(tmp.path()).items().unwrap().is_empty());
    }
    ...
    // Verifies: REQ020 / #313 — the Provreq adapter writes provenance back as a review-log
    ...
    fn annotate_appends_and_does_not_replace() {
        ...
        let before = provreq_model::load::artifact::load_content_artifact(
            &root.join("artifacts/req/REQ-queueIsDrained.md"),
        )
        .unwrap()
        .metadata
        ...
        let after = provreq_model::load::artifact::load_content_artifact(
            &root.join("artifacts/req/REQ-queueIsDrained.md"),
        )
        .unwrap()
        .metadata
    ...
    fn record_verdict_rejects_unknown_item() {
        let (_tmp, root) = subject_with_fixture_artifact();
        let err = ProvreqSource::new(&root)
