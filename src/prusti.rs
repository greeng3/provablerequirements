...
/// The harness function/module name for a requirement id — a valid Rust identifier, prefixed so
/// it cannot collide with the subject's own items.
...
/// without Prusti installed, which is what lets CI prove the engine-absent path continuously
/// (R-eng-2).
///
/// Like Creusot's (and unlike Kani's `tests/` crate), the harness is a module *inside* the
/// subject crate, so it reaches the subject's items through `crate::…`.
///
/// `// ponytail: the claim-lowering below (lower_property/lower_expr/lower_atom/sort_target)
/// mirrors kani.rs and creusot.rs — same binding rules, different wrapper. This is the THIRD
/// copy, which trips the rule of three; a shared claim-lowering module is now justified and is
/// the natural next refactor (extract once, under its own ticket, so all three suites move
/// together). Not folded into this slice: it would put three engines' behaviour behind one
/// change and risk their suites for no shipped value.`
pub fn lower(
    req: &Requirement,
    bindings: &[Binding],
...
/// Lower one `require` claim into a `prusti_assert!` statement. A quantified claim becomes a
/// `forall` closure, which is what makes it a ∀ proof over the sort rather than a spot check.
fn lower_property(
    prop: &Property,
    bindings: &[Binding],
    resolutions: &BTreeMap<String, Resolution>,
    ...
    // can arrive. The match stays total anyway: `lower` is public and must not depend on a
    // caller having gated first.
    ...
    // A quantified claim is a `forall` closure over the sort's type; an unquantified one (e.g.
    // `never overdrawn`) asserts the ground fact directly.
    ...
    Ok(format!("    prusti_assert!({assertion});\n"))
}
...
/// (the harness is in-crate). The predicate must be a `#[pure]` function for Prusti to unfold it
/// inside a `prusti_assert!`; if it is an ordinary program `fn`, the harness will not verify (or
/// will not compile) and the verdict is honestly `inconclusive` (never a wrong verdict). `syn`
/// reads syntax, not types, so a parameter-type-vs-sort mismatch also surfaces as a compile error
/// → `inconclusive`.
fn lower_atom(
    a: &Atom,
    quantifier: Option<&Quantifier>,
