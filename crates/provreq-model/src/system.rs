...
//!
//! The System config is optional: when `PROVREQ_SYSTEM_CONFIG` is
//! unset (or points at a non-existent path), Provreq runs in
...
pub enum LoadedSystem {
    /// No `PROVREQ_SYSTEM_CONFIG` provided, or the path doesn't
    /// exist. Provreq runs without a named System.
    Unnamed,
