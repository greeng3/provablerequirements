...
//!
//! Walks every Provreq-authored file inside one Project,
...
//! - Provreq never commits. The operator is expected to stage
...
/// is always available wherever Provreq runs against a real
...
fn migrate_project_config(
    ...
    let path = project_root.join("provreq.json");
    if !path.exists() {
        return;
    }
...
fn walk_collections(
    ...
    let config_path = project_root.join("provreq.json");
...
fn migrate_collection(
    ...
    for entry in entries.flatten() {
        ...
        } else if name.ends_with(".provreq.json") {
