...
mod tests {
    ...
    fn migration_copies_env_var_value_into_api_key_when_set() {
        ...
        // test isolation.
        unsafe { std::env::set_var("PROVREQ_TEST_MIG_KEY_A", "sk-from-env") };
        let v = json!({
            "schemaVersion": 1,
            "llm": [
                {
                    ...
                    "apiKeyEnvVar": "PROVREQ_TEST_MIG_KEY_A"
                }
            ]
        });
        let out = run_step(v);
    ...
    fn migration_drops_env_var_field_when_env_is_unset() {
        unsafe { std::env::remove_var("PROVREQ_TEST_MIG_KEY_B") };
        let v = json!({
            "schemaVersion": 1,
            "llm": [
                {
                    ...
                    "apiKeyEnvVar": "PROVREQ_TEST_MIG_KEY_B"
                }
            ]
        });
        let out = run_step(v);
    ...
    fn migration_preserves_existing_api_key() {
        unsafe { std::env::set_var("PROVREQ_TEST_MIG_KEY_C", "should-not-overwrite") };
        let v = json!({
            "schemaVersion": 1,
            "llm": [
                {
                    ...
                    "model": "claude-haiku-4-5",
                    "apiKey": "existing-key",
                    "apiKeyEnvVar": "PROVREQ_TEST_MIG_KEY_C"
                }
            ]
        });
        let out = run_step(v);
