...
//!   `DES-spec.pdf.provreq.json`. The binary keeps its original
//!   filename (including extension) so operators and git diff see
//!   the familiar name; the sidecar stacks `.provreq.json` on top.
//! - A URL artifact has no binary peer; it's a single
//!   `<name>.provreq.json` that carries shape / URL / check state.
//! - Filename stems derived from sidecars strip the full
//!   `.provreq.json` suffix — a blob's name is `DES-spec`, not
...
pub const SIDECAR_SUFFIX: &str = ".provreq.json";

/// Return the blob-sidecar path for a binary at `blob_path`:
/// `/foo/DES-spec.pdf` → `/foo/DES-spec.pdf.provreq.json`.
...
/// sidecar name doesn't end in `.provreq.json` or is a URL
...
pub fn blob_path_for_sidecar(sidecar_path: &Path) -> Option<PathBuf> {
    ...
    // A stem with no '.' is a URL artifact — `RFC-9110.provreq.json`
...
/// both suffixes (`DES-spec.pdf.provreq.json` → `DES-spec`); for
/// URL artifacts this strips just the sidecar suffix
/// (`RFC-9110.provreq.json` → `RFC-9110`).
...
/// Is this a sidecar path (ends in `.provreq.json`)?
...
mod tests {
    ...
    fn sidecar_path_appends_suffix_to_blob_filename() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/foo/DES-spec.pdf.provreq.json"),
        );
    }

    #[test]
    fn blob_path_from_sidecar_is_the_inverse() {
        let sidecar = PathBuf::from("/foo/DES-spec.pdf.provreq.json");
        assert_eq!(
    ...
    fn blob_path_from_url_sidecar_is_none() {
        ...
        let sidecar = PathBuf::from("/foo/RFC-9110.provreq.json");
    ...
    fn url_sidecar_filename_uses_suffix() {
        assert_eq!(url_sidecar_filename("RFC-9110"), "RFC-9110.provreq.json");
    }

    #[test]
    fn is_sidecar_path_detects_suffix() {
        assert!(is_sidecar_path(Path::new("x.provreq.json")));
        assert!(is_sidecar_path(Path::new("x.pdf.provreq.json")));
