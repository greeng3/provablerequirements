...
//! `PROVREQ_MOUNT_PREFIX` against the validity states defined in
...
/// fallback for directories that pass the `.git` + `provreq.json`
/// existence check but fail to load (for example, a corrupt
/// `provreq.json`). The spec doesn't enumerate this explicitly,
/// but surfacing it is strictly better than silently dropping the
/// mount.
#[derive(Debug)]
pub enum MountState {
    /// Both `.git/` and `provreq.json` present; load succeeded.
    Project(LoadedProject),
    /// `.git/` present, `provreq.json` missing — "not yet a
    /// Provreq project" with an init option in the UI.
    NeedsInit,
...
pub fn discover_mounts(mount_prefix: &Path) -> Result<Vec<MountInfo>, MountDiscoveryError> {
    ...
    for entry in entries {
        ...
        // Skip hidden directories (e.g. `.provreq-workspace`,
        // `.git`, `.idea`). Operators commonly co-locate Provreq's
...
/// Single-subject classify where the git repo and the Provreq project can live at
/// different paths. Provreq's multi-project model assumes `.git` and `provreq.json`
/// sit in the same mount directory; provreq self-hosts with `.git` at the repo root and
/// `provreq.json` in a `requirements/` subdir, so it resolves the two roots separately
...
/// Shared core: `.git` is looked for under `git_root`, `provreq.json` under `project_root`.
/// When the two are the same path this is exactly Provreq's original per-mount rule.
fn classify_at(project_root: PathBuf, git_root: &Path) -> MountInfo {
    let has_git = git_root.join(".git").exists();
    let has_config = project_root.join("provreq.json").exists();
...
mod tests {
    ...
    fn discover_mounts_skips_hidden_directories() {
        ...
        fs::create_dir(tmp.path().join(".provreq-workspace")).unwrap();
    ...
    fn classify_single_loads_a_project_with_git_and_config_at_different_roots() {
        // provreq's layout: `.git` at the repo root, `provreq.json` + artifacts in a
        ...
        fs::write(
            project_root.join("provreq.json"),
    ...
    fn git_root_is_the_project_dir_when_git_is_co_located() {
        // Provreq's own layout: `.git` and `provreq.json` in the same directory. The walk finds
