...
//!
//! Absorbed from Provreq (#348): the `World` struct came from its `app.rs` and `run_discovery`
//! from its `discovery.rs`. `AppState` (which wrapped `World` in axum/tokio state — `RwLock`,
//! `broadcast::Sender`, the LLM runtime) is deliberately left behind in Provreq's server; only the
...
/// The in-memory view Provreq builds from discovery. Replaced
...
pub struct DiscoveryConfig {
    ...
    /// `PROVREQ_MAX_BLOB_BYTES` when their content needs more
    ...
    /// `PROVREQ_THUMBNAIL_CACHE_MAX_BYTES`. LRU-evicted when the
    ...
    /// relative paths (resolve when re-served through Provreq,
    /// break when opened offline). Operators with a real
    /// deployment URL set `PROVREQ_EXTERNAL_URL=https://
    /// provreq.example.com`.
...
/// `.git` lives); `project_root` is where the Provreq project (`provreq.json` + artifacts) lives,
/// which provreq resolves through the companion's `subject_requirements` — for its own repo these
/// differ (`.git` at the root, `provreq.json` in `requirements/`). When they're the same path this
/// is exactly Provreq's original single-directory classification.
...
/// for every sibling project (Provreq's multi-project model); provreq uses [`discover_single`].
