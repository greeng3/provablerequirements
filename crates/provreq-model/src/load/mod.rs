...
//! `provreq.json`), plus a vector of per-artifact / per-collection
...
pub enum LoadDiagnostic {
    ...
    /// `.provreq.json` sidecar. Surfaces as a soft failure so a
    ...
    /// Provreq knows how to read, per `STOR-schemaNewerFilesRefused`.
