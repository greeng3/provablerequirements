...
//! `DES-spec.pdf`) and the sidecar (`DES-spec.pdf.provreq.json`).
...
/// verified to end in `.provreq.json`. Derives the binary path
...
mod tests {
    ...
    fn rejects_missing_binary_peer() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let sidecar_path = root.join("DES-spec.pdf.provreq.json");
    ...
    fn rejects_blob_path_that_escapes_project_root() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let sidecar_path = root.join("evil.pdf.provreq.json");
    ...
    fn rejects_blob_path_that_is_absolute() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let sidecar_path = root.join("abs.pdf.provreq.json");
