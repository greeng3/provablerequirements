//! Project-level loader — orchestrates reading `provreq.json` plus
...
impl LoadedProject {
    ...
    /// `.git` ancestor — exactly how git itself locates the repo for a path. Provreq's
    ...
    /// build a `root/.git` path and let the open fail keep working). For a co-located Provreq
...
pub enum ProjectLoadError {
    #[error("provreq.json not found at {}", path.display())]
    ConfigMissing { path: PathBuf },

    #[error("i/o error reading provreq.json at {}: {source}", path.display())]
    ConfigIo {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("invalid provreq.json at {}: {source}", path.display())]
    ConfigParse {
        path: PathBuf,
        #[source]
...
///
/// `root` is the directory containing `provreq.json`. Soft failures
...
pub fn load_project(root: &Path) -> Result<LoadedProject, ProjectLoadError> {
    let config_path = root.join("provreq.json");
...
fn load_collection(
    ...
    // Phase 5a: walk `.provreq.json` sidecars for blob + URL
...
fn immediate_subdirectories(dir: &Path) -> std::io::Result<Vec<PathBuf>> {
    ...
    for entry in fs::read_dir(dir)? {
        ...
        // (Provreq sidecars under `.suggestions/`, IDE dirs,
