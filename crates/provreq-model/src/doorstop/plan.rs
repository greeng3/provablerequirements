...
use crate::doorstop::ids::provreq_name_from_uid;
...
pub struct PlanArtifact {
    ...
    pub uuid: Uuid,
    /// Ultimate Provreq artifact name (`<prefix>-<nanu>` with
...
pub struct PlanArtifactLink {
    ...
    /// `TRACE-unresolvedLinks` can surface it in Provreq
    /// post-import.
    pub unresolved: bool,
}
...
/// Build an import plan. `project` is the target Provreq
...
pub fn build_plan(
    ...
    for doc in &documents {
        ...
        for item in &doc.items {
            let Some(name) = provreq_name_from_uid(&item.uid, &doc.settings.prefix, sep) else {
