//! Doorstop-UID → Provreq-name mapping (INTEROP-doorstopIdNormalization).
//!
...
//! identifier has three payoffs: a subject migrating onto Provreq keeps every id (the filename
...
/// The Provreq artifact name for an imported doorstop item — the UID verbatim. Returns `None` when
...
pub fn provreq_name_from_uid(uid: &str, prefix: &str, sep: &str) -> Option<String> {
...
mod tests {
    ...
    fn name_is_the_source_uid_verbatim() {
        ...
        // Provreq without any id churn: the filename stem stays exactly what it was.
        assert_eq!(
            provreq_name_from_uid("REQ001", "REQ", "").as_deref(),
            Some("REQ001")
        );
        assert_eq!(
            provreq_name_from_uid("REQ-001", "REQ", "-").as_deref(),
            Some("REQ-001")
        );
    }
    ...
    fn multi_word_nanu_is_preserved_not_underscored() {
        ...
        assert_eq!(
            provreq_name_from_uid("DES-rocket-nozzle", "DES", "-").as_deref(),
    ...
    fn uid_without_expected_prefix_is_none() {
        assert!(provreq_name_from_uid("XYZ-001", "REQ", "-").is_none());
    }

    #[test]
    fn uid_with_prefix_but_no_nanu_is_none() {
        // Prefix alone isn't an item.
        assert!(provreq_name_from_uid("REQ-", "REQ", "-").is_none());
        assert!(provreq_name_from_uid("REQ", "REQ", "").is_none());
    }
