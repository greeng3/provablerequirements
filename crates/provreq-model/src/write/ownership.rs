...
//! repository's `.git` entry so Provreq-created files don't end
...
//!    — the server reads `PROVREQ_UID` / `PROVREQ_GID` once at
...
//! the chown attempt fails and Provreq logs the failure at debug
...
/// Per-process UID/GID overrides derived from `PROVREQ_UID` /
/// `PROVREQ_GID`. The server reads these once at startup and
...
impl OwnershipOverrides {
    /// Read `PROVREQ_UID` and `PROVREQ_GID` from the process
    ...
    pub fn from_env() -> Result<Self, OwnershipError> {
        Ok(Self {
            uid: read_u32_env("PROVREQ_UID")
                .map_err(|raw| OwnershipError::InvalidUidEnv { raw })?,
            gid: read_u32_env("PROVREQ_GID")
...
pub enum OwnershipError {
    ...
    #[error("invalid PROVREQ_UID value '{raw}': not a non-negative integer")]
    InvalidUidEnv { raw: String },

    #[error("invalid PROVREQ_GID value '{raw}': not a non-negative integer")]
...
fn reconcile_ownership_impl(
    ...
    // Provreq process and .git already share an owner.
