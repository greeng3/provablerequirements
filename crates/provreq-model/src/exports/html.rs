...
//! re-serve) or absolute (via `PROVREQ_EXTERNAL_URL`).
...
<head>\n\
<meta charset=\"utf-8\">\n\
<title>Provreq · {title} · {scope}</title>\n\
<style>{css}</style>\n\
</head>\n\
<body>\n\
<h1>{title}</h1>\n\
...
/// Provreq; breaks gracefully offline).
...
fn render_filesystem_orphans(report: &FilesystemOrphansReport) -> String {
    ...
    if !report.missing_binary.is_empty() {
        body.push_str(&format!(
            "<h2 class=\"section-h2\">Sidecars whose blob is missing ({})</h2>\n<p class=\"summary\">Provreq will never delete these automatically. Restore the binary via git / filesystem, or delete the sidecar manually.</p>\n<table>\n<thead><tr><th>Sidecar</th><th>Collection</th><th>Declared blob path</th></tr></thead>\n<tbody>\n",
